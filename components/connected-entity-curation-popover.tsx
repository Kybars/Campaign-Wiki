...
function ConnectedEntityCurationPopover({ campaignId, relatedEntity, onVisibilityChange, children }: { campaignId: string; relatedEntity: CuratedEntity; onVisibilityChange: (visibility: KnowledgeVisibility) => void; children: ReactNode }) {
  ...
  return <div className="relative" onFocus={() => setOpen(true)} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} ref={rootRef}>
    <div className="flex items-center gap-2"><h3 className="min-w-0"><Link className="font-serif text-xl font-semibold text-[var(--accent)] underline decoration-[var(--line)] underline-offset-2 hover:decoration-[var(--accent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" href={campaignHref(`/campaigns/${campaignId}/entities/${relatedEntity.id}`, "dm")}>{relatedEntity.name}</Link></h3><button aria-expanded={open} aria-haspopup="dialog" aria-label={`Manage ${relatedEntity.name}`} className="rounded border border-[var(--line)] px-1.5 py-0.5 text-xs font-semibold text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--ink)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" onClick={() => setOpen((value) => !value)} type="button">Manage</button></div>
    {children}
    <div aria-hidden={!open} aria-label={`Curation for ${relatedEntity.name}`} className={open ? "absolute left-0 z-30 mt-2" : "hidden"} role="dialog"><div className="w-72 rounded-lg border border-[var(--line)] bg-[var(--paper)] p-3 text-sm text-[var(--ink)] shadow-lg"><div className="flex items-center justify-between gap-3"><p className="font-serif text-lg font-semibold">{relatedEntity.name}</p><span className="flex items-center gap-2 text-xs text-[var(--muted)]">Player visible<EntityVisibilityToggle campaignId={campaignId} entityId={relatedEntity.id} initialVisibility={visibility} label={relatedEntity.name} onVisibilityChange={onVisibilityChange} /></span></div><p className="mt-0.5 text-xs text-[var(--muted)]">{ENTITY_TYPE_SINGULAR_LABELS[relatedEntity.type]} · {prominenceGroupLabel(prominenceGroup(relatedEntity.prominence))}</p>{relatedEntity.summary ? <p className="mt-2 line-clamp-2 text-xs leading-5 text-[var(--muted)]">{relatedEntity.summary}</p> : null}</div></div>
  </div>;
}
