...
export function requiresTypeChangeConfirmation(currentType: EntityType, nextType: EntityType) {
  return currentType !== nextType;
}

export function EntityCurationControls({ campaignId, entity }: { campaignId: string; entity: { id: string; name: string; type: EntityType; prominence: EntityProminence | null; quest_status: QuestStatus | null } }) {
  ...
  const [openMenu, setOpenMenu] = useState<"prominence" | "type" | "status" | null>(null);
  const [error, setError] = useState("");
  ...
  const chooseType = (nextType: EntityType) => {
    setOpenMenu(null);
    if (requiresTypeChangeConfirmation(type, nextType)) setPendingType(nextType);
  };
  const metadataButtonClass = "rounded px-1 -mx-1 font-medium text-[var(--muted)] transition-colors hover:bg-[var(--line)] hover:text-[var(--ink)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]";
  const menuClass = "absolute left-0 top-full z-30 mt-1 min-w-32 rounded-md border border-[var(--line)] bg-[var(--paper)] p-1 text-sm shadow-lg";
  return <div className="mt-3 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm">
    <span className="relative"><button aria-expanded={openMenu === "prominence"} aria-haspopup="menu" aria-label="Edit prominence" className={metadataButtonClass} onClick={() => setOpenMenu(openMenu === "prominence" ? null : "prominence")} type="button">{(prominence ?? "minor").replace(/^./, (letter) => letter.toUpperCase())}</button>{openMenu === "prominence" ? <span className={menuClass} role="menu">{(["major", "supporting", "minor"] as const).map((value) => <button aria-checked={(prominence ?? "minor") === value} className="block w-full rounded px-2 py-1 text-left hover:bg-[var(--line)] focus:outline-none focus-visible:bg-[var(--line)]" key={value} onClick={() => { setOpenMenu(null); void patch("prominence", value); }} role="menuitemradio" type="button">{value.replace(/^./, (letter) => letter.toUpperCase())}</button>)}</span> : null}</span>
    <span className="relative"><button aria-expanded={openMenu === "type"} aria-haspopup="menu" aria-label="Edit entity type" className={metadataButtonClass} onClick={() => setOpenMenu(openMenu === "type" ? null : "type")} type="button">{ENTITY_TYPE_SINGULAR_LABELS[type]}</button>{openMenu === "type" ? <span className={menuClass} role="menu">{ENTITY_TYPES.map((value) => <button aria-checked={type === value} className="block w-full rounded px-2 py-1 text-left hover:bg-[var(--line)] focus:outline-none focus-visible:bg-[var(--line)]" key={value} onClick={() => chooseType(value)} role="menuitemradio" type="button">{ENTITY_TYPE_SINGULAR_LABELS[value]}</button>)}</span> : null}</span>
    {type === "quest" ? <span className="relative"><button aria-expanded={openMenu === "status"} aria-haspopup="menu" aria-label="Edit quest status" className={metadataButtonClass} onClick={() => setOpenMenu(openMenu === "status" ? null : "status")} type="button">{(status ?? "not_started") === "not_started" ? "Not started" : (status ?? "not_started").replace(/^./, (letter) => letter.toUpperCase())}</button>{openMenu === "status" ? <span className={menuClass} role="menu">{(["ongoing", "not_started", "finished"] as const).map((value) => <button aria-checked={(status ?? "not_started") === value} className="block w-full rounded px-2 py-1 text-left hover:bg-[var(--line)] focus:outline-none focus-visible:bg-[var(--line)]" key={value} onClick={() => { setOpenMenu(null); void patch("quest_status", value); }} role="menuitemradio" type="button">{value === "not_started" ? "Not started" : value.replace(/^./, (letter) => letter.toUpperCase())}</button>)}</span> : null}</span> : null}
    {error ? <p className="basis-full text-xs text-red-700" role="alert">{error}</p> : null}
