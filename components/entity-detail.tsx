...
import { EntityCurationControls, RelationshipVisibilityToggle } from "@/components/curation-controls";
...
type RelatedEntity = EntityPreview & { visibility?: KnowledgeVisibility };
...
function Connections({ campaignId, entity, relationships, viewMode, hideContainment }: { campaignId: string; entity: EntityDetailView["entity"]; relationships: Relationship[]; viewMode: CampaignViewMode; hideContainment: boolean }) {
  ...
  return <section className="mt-10" aria-labelledby="connections-heading"><h2 className="border-b border-[var(--line)] pb-3 font-serif text-2xl font-semibold" id="connections-heading">Connections</h2>{!remaining.length ? <p className="mt-4 text-[var(--muted)]">No supported relationships were found.</p> : <ul className="space-y-5 pt-4">{[...grouped.values()].map((group) => { const related = group[0].relatedEntity!; return <li className="rounded-lg border border-[var(--line)] bg-white/45 p-4" key={related.id}><h3 className="font-serif text-xl font-semibold"><EntityPreviewLink campaignId={campaignId} entity={related} viewMode={viewMode} /></h3><ul className="mt-2 space-y-2">{group.map((relationship) => { const blockers = relationshipVisibilityBlockers({ name: entity.name, visibility: entity.visibility ?? "dm_only" }, { name: related.name, visibility: related.visibility ?? "dm_only" }); return <li className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm" key={relationship.id}><span className="min-w-0 flex-1 text-[var(--muted)]"><span className="font-semibold text-[var(--ink)]">{sentenceCase(relationship.displayLabel)}</span>{relationship.description.trim() ? <span> — {relationship.description.trim()}</span> : null} <Citation anchors={[entity.name, related.name]} label={relationship.displayLabel} sources={relationship.sources} /></span>{viewMode === "dm" ? <RelationshipVisibilityToggle blockers={blockers} campaignId={campaignId} initialVisibility={relationship.visibility ?? "dm_only"} relationshipIds={relationship.relationshipIds ?? [relationship.id]} /> : null}</li>; })}</ul></li>; })}</ul>}</section>;
}
