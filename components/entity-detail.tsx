...
import { EntityCurationControls, EntityVisibilityToggle } from "@/components/curation-controls";
...
interface Relationship { id: string; relationshipIds?: string[]; description: string; displayLabel: string; relationship_type?: string; visibility?: KnowledgeVisibility; relatedEntity?: RelatedEntity; sourceEntity: RelatedEntity; targetEntity: RelatedEntity; sources: SourceEvidence[]; }
...
function Connections({ campaignId, entity, relationships, viewMode, hideContainment }: { campaignId: string; entity: EntityDetailView["entity"]; relationships: Relationship[]; viewMode: CampaignViewMode; hideContainment: boolean }) {
  ...
  return <section className="mt-10" aria-labelledby="connections-heading"><h2 className="border-b border-[var(--line)] pb-3 font-serif text-2xl font-semibold" id="connections-heading">Connections</h2>{!remaining.length ? <p className="mt-4 text-[var(--muted)]">No supported relationships were found.</p> : <ul className="space-y-5 pt-4">{[...grouped.values()].map((group) => { const related = group[0].relatedEntity!; const relationshipRows = <ul className="mt-2 space-y-2">{group.map((relationship) => <li className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm" key={relationship.id}><span className="min-w-0 flex-1 text-[var(--muted)]">{relationship.sourceEntity.id === entity.id ? <span className="font-semibold text-[var(--ink)]">{relationship.sourceEntity.name}</span> : <EntityLink campaignId={campaignId} entity={relationship.sourceEntity} viewMode={viewMode} />} {relationship.displayLabel} {relationship.targetEntity.id === entity.id ? <span className="font-semibold text-[var(--ink)]">{relationship.targetEntity.name}</span> : <EntityLink campaignId={campaignId} entity={relationship.targetEntity} viewMode={viewMode} />}. <Citation anchors={[relationship.sourceEntity.name, relationship.targetEntity.name]} label={relationship.displayLabel} sources={relationship.sources} /></span></li>)}</ul>; return viewMode === "dm" ? <ConnectedEntityConnectionCard campaignId={campaignId} currentEntity={entity} initialRelatedEntity={related} key={related.id} relationships={group} /> : <li className="rounded-lg border border-[var(--line)] bg-white/45 p-4" key={related.id}><h3 className="font-serif text-xl font-semibold"><EntityPreviewLink campaignId={campaignId} entity={related} viewMode={viewMode} /></h3>{relationshipRows}</li>; })}</ul>}</section>;
}
