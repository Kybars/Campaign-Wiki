...
async function summarizeBatches(parse: EnrichmentParse, system: string, input: ReturnType<typeof buildEntitySummaryInput>, name: string, usage: ModelCallUsage[]) {
...
export async function enrichCanonicalGraphWithAI(graph: CanonicalGraph, options: { parse?: EnrichmentParse; provider?: StructuredModelProvider } = {}): Promise<{ graph: CanonicalGraph; output: CampaignEnrichmentOutput; usage: ModelCallUsage[] }> {
  ...
  const parse = options.parse ?? providerParse(provider!); const usage: ModelCallUsage[] = [];
  try {
    const entityInput = buildEntityClassificationInput(graph);
    const entities = await exactBatch<z.infer<typeof entityClassificationSchema>>(parse, usage, ENTITY_CLASSIFICATION_SYSTEM_PROMPT, entityInput, entityClassificationSchema, "entity_classification", "entity_key", graph.entities.map((entity) => entity.key), "entity classification", 1, (output) => output.entities, (output) => {
    ...
    for (let index = 0; index < factsInput.length; index += FACT_CLASSIFICATION_BATCH_SIZE) { const batchInput = factsInput.slice(index, index + FACT_CLASSIFICATION_BATCH_SIZE); const result = await exactBatch<z.infer<typeof factVisibilitySchema>>(parse, usage, FACT_VISIBILITY_SYSTEM_PROMPT, { facts: batchInput }, factVisibilitySchema, "fact_visibility", "fact_key", batchInput.map((fact) => fact.key), "fact classification", Math.floor(index / FACT_CLASSIFICATION_BATCH_SIZE) + 1, (output) => output.facts); facts.push(...result.facts); }
