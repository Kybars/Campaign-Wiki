...
export function resolveOpenAIModels<T extends { OPENAI_MODEL: string; OPENAI_EXTRACTION_MODEL?: string; OPENAI_EXTRACTION_INVENTORY_MODEL?: string; OPENAI_EXTRACTION_RICH_MODEL?: string; OPENAI_GRAPH_EXTRACTION_MODEL?: string; OPENAI_GRAPH_COMPLETENESS_MODEL?: string; OPENAI_RECONCILIATION_MODEL?: string; OPENAI_ENRICHMENT_MODEL?: string }>(env: T) {
...
export type AIStage = "extraction" | "extraction_inventory" | "extraction_rich" | "graph_extraction" | "graph_completeness" | "reconciliation" | "enrichment";
...
export function resolveAIProviderConfig(env: Record<string, string | undefined>, stage: AIStage): ResolvedAIProviderConfig {
  ...
  const stageModel = stage === "extraction_inventory" ? env.LOCAL_AI_EXTRACTION_INVENTORY_MODEL || extractionModel : stage === "extraction_rich" ? env.LOCAL_AI_EXTRACTION_RICH_MODEL || extractionModel : stage === "graph_extraction" ? env.LOCAL_AI_GRAPH_EXTRACTION_MODEL || extractionModel : stage === "graph_completeness" ? env.LOCAL_AI_GRAPH_COMPLETENESS_MODEL || extractionModel : stage === "extraction" ? extractionModel : stage === "reconciliation" ? env.LOCAL_AI_RECONCILIATION_MODEL : env.LOCAL_AI_ENRICHMENT_MODEL;
