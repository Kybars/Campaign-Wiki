...
async function main() {
  ...
  console.log(JSON.stringify({ mode: "read-only cached Test 3 extraction workload", architecture: "historical single-pass", writes: 0, modelCalls: 0, chunkCount: chunks.length, entityCandidates: aggregate.entities.length, factCandidates: aggregate.entities.reduce((sum, entity) => sum + (entity.facts?.length ?? 0), 0), relationshipCandidates: aggregate.relationships.length, graphFingerprint, serializedOutput: { raw: { characters: total("characters", "raw"), utf8Bytes: total("utf8Bytes", "raw") }, validated: { characters: total("characters", "validated"), utf8Bytes: total("utf8Bytes", "validated") }, averageValidatedUtf8BytesPerChunk: Math.round(total("utf8Bytes", "validated") / chunks.length), largestValidatedChunk: largest }, collections, chunks }, null, 2));
}
