alter type public.entity_type add value if not exists 'deity' after 'npc';

alter table public.entities
  add column if not exists roles text[] not null default '{}'::text[];

alter table public.extraction_cache_runs
  alter column cache_schema_version set default 2;

create index if not exists entities_roles_idx on public.entities using gin (roles);
...
begin
  ...
  loop
    ...
    insert into public.entities (
      id, campaign_id, name, normalized_name, type, roles, aliases, summary, reconciliation_metadata
    ) values (
      generated_id,
      p_campaign_id,
      entity_record->>'name',
